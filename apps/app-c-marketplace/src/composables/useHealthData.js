...
import { checkPolicy, fetchDataPolicyForDisplay, } from '@dtou-demo/dtou-client';
import { APP_C_POLICY } from '../policy';
export function useHealthData() {
    const loading = ref(true);
    const error = ref(null);
    const dataPolicies = ref([]);
    ...
    onMounted(async () => {
        try {
            ...
            const display = await fetchDataPolicyForDisplay('http://localhost:3000/alice/health/heartrate/2024-03-01.ttl');
